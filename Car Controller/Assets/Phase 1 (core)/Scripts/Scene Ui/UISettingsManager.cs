...
{  
    ...
    {  
        ...
        SettingsManager.SaveSettings(newAutoState, SettingsManager.CurrentControl, SettingsManager.SelectedCarIndex);  
    ...
    {  
        SettingsManager.SaveSettings(SettingsManager.IsAutomatic, SettingsManager.ControlType.Buttons, SettingsManager.SelectedCarIndex);  
    ...
    {  
        SettingsManager.SaveSettings(SettingsManager.IsAutomatic, SettingsManager.ControlType.Steering, SettingsManager.SelectedCarIndex);  
    ...
    {  
        // 1. Notify the spawned vehicle of the updated transmission mode  
        ...
        // 2. --- FIXED --- Notify the Canvas UIManager to immediately swap out button/steering layouts  
        UIManager uiManager = Object.FindFirstObjectByType<UIManager>();  
        if (uiManager != null)  
        {  
            uiManager.UpdateControlLayoutUI();  
        }

        // 3. Notify the HUD to instantly hide/show manual shifting panels  
        VehicleHUD hud = Object.FindFirstObjectByType<VehicleHUD>();  
        if (hud != null && car != null)  
        {  
            hud.SetCar(car);  
        }  
    }
